...
export class SpaceService {
  ...
  async getSpaceById(spaceId: string) {
    try {
      ...
      throw new InternalServerErrorException(
        `Hubo un error al obtener el espacio. Error: ${error.message}`,
      );
    }
  }
    ...
    try {
      ...
      if (!spaceName)
        throw new NotFoundException(
          'No se encontro ningun espacio con este nombre.',
        );
      return spaceName;
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
  ...
  async getAllSpace(page: number, limit: number): Promise<Space[]> {
    try {
      ...
      if (!spaces.length)
        throw new NotFoundException('Lo lamentamos no hay espacios aun.');
