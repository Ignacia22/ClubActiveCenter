...
export class ConfirmationHour implements ValidatorConstraintInterface {
  validate(
    ...
    if(hour <= 7 && minute <= 59 || hour >= 1 && minute >= 30 && daysOfWeek === 0) throw new BadRequestException('No se puede hacer una actividad antes de la apertura a las 7 am y tampoco una despues del cierre a las 01:30 am.');
    if(hour <= 7 && minute <= 59 || hour >= 22 && minute >= 30) throw new BadRequestException('No se puede hacer una actividad antes de la apertura a las 7 am y tampoco una despues del cierre a las 10:30 pm.');
    return true;
  }
}
